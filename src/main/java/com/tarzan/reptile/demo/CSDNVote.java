...
public class CSDNVote {
    ...
    static {
        ...
        commentList.add("9级原力，5星已投，诚信互助： 投票后有大额红包可领取 \n" +
               ...
               if(html.contains("2022年「博客之星」参赛博主")){
                 String name =driver.findElement(By.className("name")).getText();
                   ...
                   if(CollectionUtils.isNotEmpty(offStarsEle)&&offStarsEle.size()>=starNum){
                       ...
                       //offFiveStar.click();
                       JavascriptExecutor offFiveStarJS= (JavascriptExecutor)driver;
                       offFiveStarJS.executeScript("arguments[0].click();",offFiveStar);
                       System.out.println("网址 "+voteSite+" 已投票"+starNum+"星");
                       Thread.sleep(RandomUtil.randomEle(Lists.newArrayList(1000,5000)));
                       WebElement commentPlugin=driver.findElement(By.id("Comment"));
                       Thread.sleep(RandomUtil.randomEle(Lists.newArrayList(1000,5000)));

                       JavascriptExecutor jse= (JavascriptExecutor)driver;
                       jse.executeScript("arguments[0].click();",commentPlugin);
